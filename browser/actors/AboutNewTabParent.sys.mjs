...
export class AboutNewTabParent extends JSWindowActorParent {
  ...
  async receiveMessage(message) {
    switch (message.name) {
      case "LykonHome:GetStatsAndTopSites": {
        const { statsMonitor } = ChromeUtils.importESModule(
          "resource:///modules/StatsMonitor.sys.mjs"
        );
        const stats = statsMonitor
          ? statsMonitor.getStats()
          : { session: 0, bytes: 0 };
        const formattedBytes = statsMonitor
          ? statsMonitor.fmtBytes(stats.bytes)
          : "0 B";
        const topSites = lazy.AboutNewTab.getTopSites();
        ...
        try {
          ...
          const channel = NetUtil.newChannel({
            uri: "resource://newtab/prerendered/wallpaper.json",
          ...
          console.error("LykonNewTab: Failed to read wallpaper.json:", e);
        }

        let settingsState = null;
        try {
          let raw = Services.prefs.getStringPref("lykon.homepage.state", "{}");
          settingsState = JSON.parse(raw);
        } catch (e) {
          console.error(e);
        }

        return { stats, formattedBytes, topSites, wallpapers, settingsState };
      }
      case "LykonHome:SaveState": {
        try {
          Services.prefs.setStringPref(
            "lykon.homepage.state",
